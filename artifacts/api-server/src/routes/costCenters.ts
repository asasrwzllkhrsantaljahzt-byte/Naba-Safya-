...
router.get("/cost-centers", async (req, res) => {
  try {
    ...
    req.log.error(err); 
    return res.status(500).json({ error: "فشل في جلب مراكز التكلفة" }); 
  }
});
...
router.post("/cost-centers", async (req, res) => {
  try {
    ...
    return res.status(201).json({ ...center, budget: parseFloat(center.budget ?? "0"), createdAt: center.createdAt.toISOString(), totalSpent: 0 });
  } catch (err) { 
    req.log.error(err); 
    return res.status(500).json({ error: "فشل في إضافة مركز التكلفة" }); 
  }
});
...
router.patch("/cost-centers/:id", async (req, res) => {
  try {
    ...
    return res.json({ ...center, budget: parseFloat(center.budget ?? "0"), createdAt: center.createdAt.toISOString(), totalSpent });
  } catch (err) { 
    req.log.error(err); 
    return res.status(500).json({ error: "فشل في تحديث مركز التكلفة" }); 
  }
});
...
router.delete("/cost-centers/:id", async (req, res) => {
  try {
    ...
    req.log.error(err); 
    return res.status(500).json({ error: "فشل في حذف مركز التكلفة" }); 
  }
});
...
router.get("/cost-centers/:id/expenses", async (req, res) => {
  try {
    ...
    req.log.error(err); 
    return res.status(500).json({ error: "فشل في جلب مصروفات matrix التكلفة" }); 
  }
});
