...
// 🟢 جينا هنا وأضفنا الـ return في النجاح والخطأ لتصفية الأخطاء الصفراء
router.get("/attendance", async (req, res) => {
  try {
    ...
    return res.json(records.map(fmt));
  } catch (err) { 
    req.log.error(err); 
    return res.status(500).json({ error: "فشل في جلب الحضور" }); 
  }
});
...
router.post("/attendance", async (req, res) => {
  try {
    ...
    req.log.error(err); 
    return res.status(500).json({ error: "فشل في إضافة سجل الحضور" }); 
  }
});
...
router.patch("/attendance/:id", async (req, res) => {
  try {
    ...
    return res.json(fmt(record));
  } catch (err) { 
    req.log.error(err); 
    return res.status(500).json({ error: "فشل في تحديث سجل الحضور" }); 
  }
});
...
router.delete("/attendance/:id", async (req, res) => {
  try {
    ...
    req.log.error(err); 
    return res.status(500).json({ error: "فشل في حذف سجل الحضور" }); 
  }
});
