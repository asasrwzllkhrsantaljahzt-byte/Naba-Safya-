...
export default function Attendance() {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      fetch(
        `${BASE}/api/attendance?month=${month}${
  ...
  const remove = useMutation({
    mutationFn: (id: number) =>
      fetch(`${BASE}/api/attendance/${id}`, { method: "DELETE" }),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["attendance"] }),
  });
  ...
  const summary = useMemo(() => {
    return employees.map((e: any) => {
      ...
      return {
        ...
        overtime: empRecords.reduce(
          (s: number, r: any) => s + (r.overtime || 0),
          0
        ),
      };
    });
              ...
              records.map((r: any) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        typeLabels[r.type]?.variant ?? "secondary"
                      }
                    >
                      {typeLabels[r.type]?.label ?? r.type}
                    </Badge>
                  </TableCell>
